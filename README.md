...
| 规则 | 说明 |
|---|---|
| 榜单只收有评分模型 | 全库 341 个中仅 131 个有第三方能力评分；无评分模型**不进任何榜单与象限图**，但在「全部模型」页可查并标注「未评分」 |
| 首页主榜 = 效率前沿榜 | 前沿 = 没有任何模型能同时做到更便宜且更强（帕累托最优），比「性价比排行榜」更接近「真正值得买」 |
| 性价比榜带能力门槛 | 首页次榜只排**能力分 ≥ 60** 的模型；否则会被「便宜但很弱」的模型霸榜（性价比 = 能力分 ÷ 混合价，分母越小数值越高） |
| 排名提示只显示一条 | 表格每行只显示与**当前排序**对应的名次，不再堆叠三个名次 |
| 筛选栏默认折叠 | 默认只露搜索 / 排序 / 厂商；其余收进「更多筛选」，URL 带高级条件时自动展开 |

> 调整的来龙去脉、6 个决策点的确认记录与验证结果见 `docs/ADJUSTMENT-PLAN.md`。
